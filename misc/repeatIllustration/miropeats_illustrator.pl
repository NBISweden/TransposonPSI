...
while (<REP>) {
	...
	my $dir_left = ($rep1_c1 < $rep1_c2) ? '+' : '-';
	my $dir_right = ($rep2_c1 < $rep2_c2) ? '+' : '-';

	my $orient = ($dir_left eq $dir_right) ? 'F' : 'R';
...
sub create_repeat_image {
	...
	my @colors = &ColorGradient::get_RGB_gradient(scalar(@repeats) + 1);
	
    foreach my $repeat (@repeats) {

		my  $arc_factor = ($repeat->{direction} eq 'F') ? 2 : 1.5;
