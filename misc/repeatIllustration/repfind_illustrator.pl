...
my $firstLine = <REP>;
my $secondLine = <REP>;
my @entries = split (/\s+/, $secondLine);
my $sequence_length = $entries[1];
...
while (<REP>) {
    ...
    chomp;
    my @x = split (/\s+/);
    my ($sizeA, $match_A_lend, $orient, $sizeB, $match_B_lend, $numEdits, $evalue) = @x; 

    
    my ($rep1_c1, $rep1_c2, $rep2_c1, $rep2_c2, $per_id) = 
	($match_A_lend, $match_A_lend + $sizeA -1, $match_B_lend, $match_B_lend + $sizeB - 1, 100);
    

    if ($orient eq "P") {
	($rep2_c1, $rep2_c2) = ($rep2_c2, $rep2_c1);
    }
